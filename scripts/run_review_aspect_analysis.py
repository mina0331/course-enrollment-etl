...
DAGS_DIR = ROOT_DIR / "etl" / "airflow" / "dags"
if str(DAGS_DIR) not in sys.path:
    sys.path.insert(0, str(DAGS_DIR))

from course_forum_reviews import clean_text, sentiment_from_text  # noqa: E402
...
def iter_reviews(conn, batch_size: int = 250):
    offset = 0
    while True:
        rows = conn.execute(
            text(
...
def run_aspect_analysis(database_url: str | None = None) -> None:
    ...
    with engine.begin() as conn:
        for review_batch in iter_reviews(conn):
            ...
            if len(review_aspect_rows) >= 500:
                ...
                print(f"Processed {processed_reviews} reviews for aspect analysis so far")
        ...
        print(
            f"Finished aspect analysis for {processed_reviews} reviews; "
            f"stored {len(summary_rows)} aspect summary rows"
        )
