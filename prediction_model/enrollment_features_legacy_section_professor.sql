WITH professor_features AS (
    SELECT
        ...
        AVG(cfrs.review_count) AS avg_course_forum_review_count,
...
section_base AS (
    SELECT
        ...
        s.meetings_end_time,
        CASE
            WHEN s.capacity > 0 THEN s.seats_taken * 1.0 / s.capacity
            ELSE NULL
        END AS fill_ratio,
        CASE
            WHEN s.capacity > 0 THEN s.current_waitlist * 1.0 / s.capacity
            ELSE NULL
        ...
        CASE
            ...
            ELSE NULL
        END AS course_level,
        CASE SUBSTR(s.term_id, LENGTH(s.term_id), 1)
            WHEN '2' THEN 'spring'
            WHEN '6' THEN 'summer'
            WHEN '8' THEN 'fall'
            ELSE 'other'
        END AS season,
        COALESCE(LENGTH(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(s.meetings_days, 'M', ''), 'T', ''), 'W', ''), 'R', ''), 'F', ''), 'S', ''), 'U', '')), 0) AS meeting_day_count_raw,
...
section_base_adjusted AS (
    SELECT
        *,
        LENGTH(COALESCE(meetings_days, '')) - COALESCE(meeting_day_count_raw, 0) AS meeting_day_count
    FROM section_base
),
feature_history AS (
    SELECT
        ...
        AVG(sb.current_waitlist * 1.0) OVER (
            PARTITION BY sb.course_id
            ORDER BY sb.term_numeric
            ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING
        ) AS avg_waitlist_prior_course,
        AVG(sb.capacity * 1.0) OVER (
            PARTITION BY sb.course_id
            ORDER BY sb.term_numeric
            ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING
